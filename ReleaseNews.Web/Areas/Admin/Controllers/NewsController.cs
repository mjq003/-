...
namespace ReleaseNews.Web.Areas.Admin.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        [HttpGet]
        public JsonResult GetNews(int pageIndex, int pageSize, int quertType, int classifyId, string keyword)
        {
            ...
            int total = 0;
            var news = _newsService.NewsPageQuery(pageSize, pageIndex, quertType, out total, wheres);
        ...
        [HttpPost]
        public JsonResult DelNewsClassify(int id)
        {
            if (id < 0)
                return Json(new ResponseModel { code = 0, result = "新闻类别不存在" });
            return Json(_newsService.DelOneNewsClassify(id));
        }
        ...
        {
            var newsClassifys = _newsService.GetAllNewsClassifyList();
